...
class _ProfilePageState extends State<ProfilePage> {
  ...
  getUserProfile() async {
    try {
      await storage.loadProfileFile(likeUser.email.toString(), likeUser.profilePic.toString()).then((value) => {
        img_url = value,
        // backgroundImg = NetworkImage(img_url),
        setState(() {
          _isImg = false;
        })
      });
  ...
  Widget build(BuildContext context) {
    ...
    // final image = NetworkImage(img_url);  // img url

    return (_isLoading && _isImg && _isBackground)? Center(child: CircularProgressIndicator(color: Theme.of(context).primaryColor,),) : Container(
      child: Column(
        children: [
          Stack(
